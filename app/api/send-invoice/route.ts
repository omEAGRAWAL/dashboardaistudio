...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!customerEmail || !pdfBase64) {
      return NextResponse.json({ error: 'Missing customerEmail or pdfBase64' }, { status: 400 });
    }
    ...
    const subject = `Invoice #${invoiceNumber} — ${packageTitle || 'Booking'} | ${agencyName || 'Your Travel Agency'}`;

    // Build a simple wrapper email HTML
    const emailHtml = `
<!DOCTYPE html>
<html lang="en">
...
<body style="margin:0;padding:0;background:#f9fafb;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif;">
  <table width="100%" cellpadding="0" cellspacing="0" style="background:#f9fafb;padding:40px 16px;">
    <tr><td align="center">
      <table width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%;">
        ...
        <tr><td style="background:linear-gradient(135deg,#4f46e5,#7c3aed);border-radius:16px 16px 0 0;padding:32px;">
          <div style="font-size:22px;font-weight:700;color:#fff;">${agencyName || 'Your Travel Agency'}</div>
        ...
        <tr><td style="background:#fff;padding:32px;border-left:1px solid #e5e7eb;border-right:1px solid #e5e7eb;">
          ...
          <div style="font-size:14px;color:#6b7280;line-height:1.6;margin-bottom:24px;">
            Hi <strong style="color:#374151;">${customerName || 'Customer'}</strong>,<br/>
            Please find your invoice for <strong style="color:#4f46e5;">${packageTitle || 'your booking'}</strong> attached as a PDF.
          </div>
          <div style="background:#f9fafb;border:1px solid #e5e7eb;border-radius:12px;padding:16px;margin-bottom:24px;">
            ...
            <div style="font-size:14px;color:#374151;margin-bottom:4px;">📦 ${packageTitle || 'N/A'}</div>
        ...
        <tr><td style="background:#f9fafb;border:1px solid #e5e7eb;border-top:none;border-radius:0 0 16px 16px;padding:20px 32px;text-align:center;">
          <div style="font-size:12px;color:#9ca3af;">
            Sent by <strong style="color:#4f46e5;">${agencyName || 'Your Travel Agency'}</strong>
          </div>
        </td></tr>
      </table>
    </td></tr>
    ...
    const info = await transporter.sendMail({
      from: process.env.SMTP_FROM || `"${agencyName || 'Travel Agency'}" <${process.env.SMTP_USER}>`,
      to: customerEmail,
      subject,
      html: emailHtml,
      attachments: [
        {
          filename: `Invoice-${invoiceNumber}-${(packageTitle || 'booking').replace(/[^a-zA-Z0-9]/g, '-')}.pdf`,
          content: pdfBuffer,
